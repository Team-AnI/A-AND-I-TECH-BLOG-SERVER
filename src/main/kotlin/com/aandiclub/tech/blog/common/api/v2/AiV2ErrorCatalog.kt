...
object AiV2ErrorCatalog {
	...
	val scheduledListOnly = descriptor(HttpStatus.BAD_REQUEST, 60402, "예약 글은 내 예약 글 목록에서만 조회할 수 있습니다.", "예약 글은 내 예약 글 목록에서만 볼 수 있어요.")
	...
	val contentRequiredForScheduled = descriptor(HttpStatus.BAD_REQUEST, 60307, "예약 상태에서는 본문 내용이 필요합니다.", "예약하려면 본문 내용을 입력해 주세요.")
	val scheduledPublishAtRequired = descriptor(HttpStatus.BAD_REQUEST, 60308, "예약 상태에서는 scheduledPublishAt 값이 필요합니다.", "예약 발행 시간을 입력해 주세요.")
	val scheduledPublishAtFuture = descriptor(HttpStatus.BAD_REQUEST, 60309, "scheduledPublishAt 값은 미래 시각이어야 합니다.", "예약 발행 시간을 현재보다 이후로 설정해 주세요.")
	val scheduledPublishAtOnlyForScheduled = descriptor(HttpStatus.BAD_REQUEST, 60310, "scheduledPublishAt 값은 예약 상태에서만 사용할 수 있습니다.", "예약 발행 시간은 예약 글에서만 설정할 수 있어요.")
