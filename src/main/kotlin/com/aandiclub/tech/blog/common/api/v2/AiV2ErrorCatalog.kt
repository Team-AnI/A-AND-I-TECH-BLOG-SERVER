...
object AiV2ErrorCatalog {
	...
	val scheduledListOnly = descriptor(HttpStatus.BAD_REQUEST, 60402, "scheduled posts are only available in my scheduled list", "scheduled posts are only available in my scheduled list")
	...
	val contentRequiredForScheduled = descriptor(HttpStatus.BAD_REQUEST, 60307, "contentMarkdown is required for scheduled post", "contentMarkdown is required for scheduled post")
	val scheduledPublishAtRequired = descriptor(HttpStatus.BAD_REQUEST, 60308, "scheduledPublishAt is required for scheduled post", "scheduledPublishAt is required for scheduled post")
	val scheduledPublishAtFuture = descriptor(HttpStatus.BAD_REQUEST, 60309, "scheduledPublishAt must be in the future", "scheduledPublishAt must be in the future")
	val scheduledPublishAtOnlyForScheduled = descriptor(HttpStatus.BAD_REQUEST, 60310, "scheduledPublishAt is only allowed for scheduled post", "scheduledPublishAt is only allowed for scheduled post")
