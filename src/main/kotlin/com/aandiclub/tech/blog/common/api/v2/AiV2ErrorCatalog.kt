...
object AiV2ErrorCatalog {
	...
	val scheduledListOnly = descriptor(HttpStatus.BAD_REQUEST, 60402, AiV2ErrorService.BLOG, AiV2ErrorCategory.BUSINESS, "SCHEDULED_LIST_ONLY", "예약 글은 내 예약 글 목록에서만 조회할 수 있습니다.", "예약 글은 내 예약 글 목록에서만 볼 수 있어요.", AiV2ErrorSeverity.LOW)
	...
	val contentRequiredForScheduled = descriptor(HttpStatus.BAD_REQUEST, 60307, AiV2ErrorService.BLOG, AiV2ErrorCategory.VALIDATION, "SCHEDULED_POST_CONTENT_REQUIRED", "예약 상태에서는 본문 내용이 필요합니다.", "예약하려면 본문 내용을 입력해 주세요.", AiV2ErrorSeverity.LOW)
	val scheduledPublishAtRequired = descriptor(HttpStatus.BAD_REQUEST, 60308, AiV2ErrorService.BLOG, AiV2ErrorCategory.VALIDATION, "SCHEDULED_PUBLISH_AT_REQUIRED", "예약 상태에서는 scheduledPublishAt 값이 필요합니다.", "예약 발행 시간을 입력해 주세요.", AiV2ErrorSeverity.LOW)
	val scheduledPublishAtFuture = descriptor(HttpStatus.BAD_REQUEST, 60309, AiV2ErrorService.BLOG, AiV2ErrorCategory.VALIDATION, "SCHEDULED_PUBLISH_AT_FUTURE", "scheduledPublishAt 값은 미래 시각이어야 합니다.", "예약 발행 시간을 현재보다 이후로 설정해 주세요.", AiV2ErrorSeverity.LOW)
	val scheduledPublishAtOnlyForScheduled = descriptor(HttpStatus.BAD_REQUEST, 60310, AiV2ErrorService.BLOG, AiV2ErrorCategory.VALIDATION, "SCHEDULED_PUBLISH_AT_ONLY_FOR_SCHEDULED", "scheduledPublishAt 값은 예약 상태에서만 사용할 수 있습니다.", "예약 발행 시간은 예약 글에서만 설정할 수 있어요.", AiV2ErrorSeverity.LOW)
	val badRequest = descriptor(HttpStatus.BAD_REQUEST, 90001, AiV2ErrorService.COMMON, AiV2ErrorCategory.GENERAL, "BAD_REQUEST", "잘못된 요청입니다.", "요청을 처리할 수 없어요. 다시 시도해 주세요.", AiV2ErrorSeverity.LOW, AiV2ErrorStatus.DEPRECATED)
	...
	val all: List<AiV2ErrorDescriptor> = listOf(
		...
		contentRequiredForScheduled,
		scheduledPublishAtRequired,
		scheduledPublishAtFuture,
		scheduledPublishAtOnlyForScheduled,
		badRequest,
		s3UploadFailed,
